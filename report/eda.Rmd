...
department_wide <- user_purchases %>%
  group_by(order_id, product_id, department_id) %>%
  summarise(product_num = n()) %>%
  pivot_wider(names_from = "department_id", values_from = "product_num") %>%
  left_join(orders, by = "order_id") %>%
  left_join(products, by = "product_id") %>%
  relocate(order_dow, .before = "product_id") %>%
  relocate(order_hour_of_day, .before = "product_id") %>%
  relocate(aisle_id, .after = "product_id")
