...
We only sub-sample the data without applying cross-validation. Now every class has the same frequency(**12627**).
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
insta.te <- insta.te %>%
  rename(canned.goods = `canned goods`) %>%
  rename(dairy.eggs = `dairy eggs`) %>%
  rename(meat.seafood = `meat seafood`) %>%
  rename(personal.care = `personal care`) %>%
  rename(dry.goods.pasta = `dry goods pasta`)
...
According to the confusion matrix the balanced accuracy is higher and the difference between sensitivity(**0.622**) and specificity(**0.480**) is even smaller. Now the Kappa is **0.098**, higher than Cohen's Kappa previous model(**0.032**). 
