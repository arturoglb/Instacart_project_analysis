...
# Separate our data into Training set and Test set
set.seed(12345) # for reproducibility
index.tr2 <- createDataPartition(y = data_group$Group, p= 0.8,list = FALSE)
df.tr2 <- data_group[index.tr2,]
df.te2 <- data_group[-index.tr2,]
...
cm<-confusionMatrix(m3_tree_group_pred, df.te2$Group)
...
df.tr2 <- data_group1[index.tr2,]
df.te2 <- data_group1[-index.tr2,]
...
# Apply Model to the test dataset
set.seed(12345)
m4_rf_group_pred <- predict(m4_rf_group, newdata=df.te2)
...
cm <-confusionMatrix(m4_rf_group_pred, df.te2$Group)
...
confusionMatrix(factor(insta_pred),df.te$order_dow)
```
...
confusionMatrix(factor(insta_pred_sub),df.te$order_dow)
```
...
```{r echo=FALSE, message=FALSE}
insta.glm_pred <- predict(insta_glm,newdata = df.tr2)
confusionMatrix(factor(insta.glm_pred),df.tr2$Group)
...
cm <-confusionMatrix(insta.glm_pred, df.te2$Group)
draw_confusion_matrix3(cm)
```

According to the confusion matrix the balanced accuracy is higher and the difference between sensitivity(**0.629**) and specificity(**0.475**) is even smaller. Now the Kappa is **0.1**, higher than Cohen's Kappa previous model(**0.032**), and the Accuracy is **0.574**. 

Comparing the result of this model against the previous ones, we note that Random forest model is the one that resembles these results, only Logistic regression is higher on the Accuracy by **0.012** and on Cohen's Kappa by **0.004** with the same Balanced Accuracy. So, we can opt choosing this model over the rest.
...
x_train2 <- select(df.tr2, -Group) # week and weekend
y_train2 <- pull(df.tr2, Group) # week and weekend
...
As we note from this chart the models Classification Tree, Random Forest, and Logistic Regression use the AUC loss to compare the model quality of shuffling different variables. AUC is a synthetic measure of the distance to random model in the ROC curve plot. The larger the curve to the top left side of the plot, the better the model.

According to the feature importance of this three models, the most important department is the products department. As we have seen above in the table of number of purchases per department, the product department is the leading one with almost twice the number of the second one in that table. This could be one of the main reasons why all three models have chosen it as the most relevant. It is important to mention that if we shuffle this variable, the AUC of the model will have the largest loss.
