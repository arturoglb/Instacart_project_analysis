...
confusionMatrix(factor(insta.glm_pred),factor(df.tr2$Group))
```
...
For this model, we use root mean square error(RMSE) to compare the model quality of shuffling different variables. According to the plot p3, the most important variable is dairy eggs. If we shuffle this variable, the RMSE of the model will have the largest increase.

* **Logistic Regression**

As in model classification tree, in logistic regression we also use the AUC loss to compare the model quality of shuffling different variables. AUC is a synthetic measure of the distance to random model in the ROC curve plot. The larger, the better the model. According to plot p4, the most important variable is produce. If we shuffle this variable, the AUC of the model will have the largest loss.
