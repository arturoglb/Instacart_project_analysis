...
According to the confusion matrix the balanced accuracy is higher and the difference between sensitivity(**0.622**) and specificity(**0.480**) is even smaller. Now the Kappa is **0.098**, higher than Cohen's Kappa previous model(**0.032**), and the Accuracy is **0.573**. 

Comparing the result of this model against the previous ones, we note that Random forest model is the one that resembles these results, only Logistic regression is higher on the Accuracy by **0.011** and on Cohen's Kappa by **0.002** with almost the same Balanced Accuracy. So, we can opt choosing this model over the rest.
