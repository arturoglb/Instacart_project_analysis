...
According to the confusion matrix the balanced accuracy is higher and the difference between sensitivity(**0.620**) and specificity(**0.472**) is even smaller. Now the Kappa is **0.089**, higher than Cohen's Kappa previous model(**0.032**), and the Accuracy is **0.574**. 

Comparing the result of this model against the previous ones, we note that Random forest model is the one that resembles these results, only Logistic regression is slightly higher on the Accuracy by **0.008**. So, we have decided choosing this model over the rest.
